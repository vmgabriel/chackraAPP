...
class BoardView extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final listData = useState<Map<entity_task.TaskStatus, List<entity_task.Task>>>({
    ...
    final taskListAsync = ref.watch(provider_task.taskListProvider(request));

    return Scaffold(
      appBar: appbar.CustomAppBar(title: "Tus Tableros"),
      body: taskListAsync.when(
        ...
        data: (paginator) {
          useEffect(() {
            final data = <entity_task.TaskStatus, List<entity_task.Task>>{
              for (var status in entity_task.TaskStatus.values)
                status: <entity_task.Task>[],
            };

            for (var task in paginator.elements) {
              data[task.status]!.add(task);
            }
          ...
          return SingleChildScrollView(
            ...
            child: Row(
              ...
              children: List.generate(listDataIndex.length, (listIndex) {
                return DragTarget<Map<String, dynamic>>(
                  ...
                  onAccept: (data) {
                    ...
                    final newData = Map<entity_task.TaskStatus, List<entity_task.Task>>.from(listData.value);
                    newData[listDataIndex[fromList]]!.removeAt(index);
                    newData[listDataIndex[listIndex]]!.add(task);
                    listData.value = newData;
                  },
                  builder: (context, candidateData, rejectedData) {
                    final tasks = listData.value[listDataIndex[listIndex]] ?? [];
                    return Container(
                      width: 280,
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          TextButton.icon(
                            onPressed: () {
                              // Aquí puedes agregar una nueva tarea
                            },
